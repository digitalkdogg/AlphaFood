...
Return a JSON object with exactly these fields:
{
  ...
  "mammal_status": <string: exactly one of "safe", "questionable", or "contains_mammal".
    ...
    - "contains_mammal": one or more ingredients are explicitly listed and are a mammal-derived product. This includes: beef, pork, veal, ham, lamb, venison, bison, buffalo, goat meat, rabbit, lard, tallow, suet, rennet, gelatin, worcestershire sauce, AND these specific dairy products — milk, butter, cream, yogurt, ghee, whey, sour cream, half-and-half, buttermilk, kefir, cream cheese, mascarpone, ricotta, creme fraiche. For bacon specifically: only flag bacon that is explicitly pork-based or completely unqualified (e.g. "bacon" alone). Do NOT flag turkey bacon, duck bacon, chicken bacon, or any bacon explicitly labeled as poultry-derived — these are safe. Only mark "contains_mammal" if the ingredient is actually present in the listed ingredients. IMPORTANT: style or flavor adjectives such as "greek", "plain", "vanilla", "low-fat", "non-fat", "full-fat", "whole", "skim", "2%", "unsalted", "salted", "whipped", "organic", "raw" do NOT make a dairy ingredient plant-based. "Greek yogurt" is still yogurt. "Unsalted butter" is still butter. Only qualifiers that explicitly indicate a plant-based source (coconut, oat, almond, soy, rice, cashew, vegan, dairy-free, plant-based) change the classification.
...
They CAN safely eat: chicken, turkey, duck, goose, and all other poultry, fish, seafood, and all plant-based ingredients.
IMPORTANT: "bacon" made from poultry (duck bacon, turkey bacon, chicken bacon) is SAFE — do not flag it. Only flag bacon that is explicitly pork-based or unqualified "bacon" with no poultry qualifier.
